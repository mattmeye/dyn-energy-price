...
ROLES: tuple[Role, ...] = (
    ...
    Role(
        ...
        label="Ladestromgrenze (DVCC/CCL)",
        hint="optional, A oder W - das BMS senkt sie bei kalten Zellen ab",
        units=frozenset({"a", "w", "kw"}),
        positive=("charge_current_limit", "ccl", "max_charge_current", "max_charge_power",
                  "ladestrom", "ladeleistung", "charge_limit"),
        negative=("discharge", "entlad", "car", "auto", "wallbox"),
    ),
    Role(
        key="battery_discharge_limit",
        label="Entladestromgrenze (DVCC/DCL)",
        hint="optional, A oder W",
        units=frozenset({"a", "w", "kw"}),
        positive=("discharge_current_limit", "dcl", "max_discharge_current",
                  "max_discharge_power", "entladestrom", "entladeleistung", "discharge_limit"),
        negative=("car", "auto", "wallbox"),
    ),
    Role(
        key="battery_voltage",
        label="Batteriespannung",
        hint="optional, V - rechnet die Stromgrenzen in Leistung um",
        units=frozenset({"v"}),
        positive=("battery_voltage", "batterie_spannung", "akku_spannung", "dc_voltage",
                  "system_battery_voltage"),
        negative=("cell", "zelle", "grid", "netz", "ac_"),
        device_classes=("voltage",),
    ),
    Role(
        key="battery_soc_min",
        label="Mindest-Ladezustand (ESS)",
        hint="optional, % - wird dann live statt fest verwendet",
        units=frozenset({"%"}),
        positive=("minimum_soc", "min_soc", "mindest", "soc_limit", "ess_minimum"),
        negative=("max", "car", "auto"),
    ),
    Role(
        key="ac_input_limit",
        label="Eingangsstrombegrenzung",
        hint="optional, A - begrenzt, was aus dem Netz durch den Wechselrichter fließt",
        units=frozenset({"a"}),
        positive=("input_current_limit", "ac_input_current", "eingangsstrom", "current_limit"),
        negative=("output", "ausgang", "charge", "discharge", "battery"),
    ),
    Role(
        key="heatpump_energy",
