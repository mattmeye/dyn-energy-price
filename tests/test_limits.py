...
def test_eingangsstrom_begrenzt_die_netzladung():
    battery = Battery(grid_charge_kw_override=5.0, ac_input_limit_a=16.0,
                      mains_voltage_v=230.0, phases=1)
    limits = resolve(FakeHass(), battery)
...
def test_dreiphasiger_anschluss_hebt_die_grenze():
    battery = Battery(mppt_charge_kw=20.0, grid_charge_kw_override=20.0,
                      ac_input_limit_a=16.0, phases=3)
    limits = resolve(FakeHass(), battery)
...
def test_netzladung_nie_ueber_der_ladeannahme():
    ...
    hass = FakeHass({"sensor.ccl": sensor("sensor.ccl", 60, "A")})   # ~3,1 kW Ladeannahme
    limits = resolve(hass, Battery(), entities)
    assert round(limits.grid_charge_kw, 2) == round(limits.charge_kw, 2)
...
def test_stabile_grenze_erzeugt_keinen_hinweis():
    entities = Entities(battery_charge_limit="sensor.ccl")
    hass = FakeHass({"sensor.ccl": sensor("sensor.ccl", 400, "A")}, {"sensor.ccl": 395.0})
...
def test_apply_uebertraegt_die_leistungen():
    ...
    assert updated.discharge_kw == limits.discharge_kw
